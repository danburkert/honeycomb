...
public class MutationFactoryTest {
    ...
    private static final Map<String, ColumnSchema> COLUMNS = new HashMap<String, ColumnSchema>() {{
        put(COLUMN1, ColumnSchema.newBuilder()
                .setType(ColumnType.LONG)
                .setIsAutoIncrement(true)
                .setIsNullable(false)
                .build());
        put(COLUMN2, ColumnSchema.newBuilder()
    ...
    private static final Map<String, IndexSchema> INDICES = new HashMap<String, IndexSchema>() {{
        put(INDEX1, IndexSchema.newBuilder()
                .setColumns(Lists.newArrayList(COLUMN1))
                .setIsUnique(false)
                .build());
        put(INDEX2,  IndexSchema.newBuilder()
                .setColumns(Lists.newArrayList(COLUMN1, COLUMN2))
                .setIsUnique(true)
                .build());
    }};
    ...
    public void testPartialInsertRejectsInvalidTableId() throws Exception {
        factory.insert(-1, row, new HashMap<String, IndexSchema>());
    }
    @Test(expected = TableNotFoundException.class)
    public void testPartialInsertRejectsUnknownTableId() throws Exception {
        factory.insert(99, row, ImmutableMap.of(INDEX1, INDICES.get(INDEX1)));
    }
    @Test(expected = NullPointerException.class)
    public void testPartialInsertRejectsNullRow() throws Exception {
        factory.insert(1, null, new HashMap<String, IndexSchema>());
    }

    @Test
    public void testPartialInsert() throws Exception {
        List<Put> puts = factory.insert(tableId, row,
                ImmutableMap.of(INDEX1, INDICES.get(INDEX1)));
        byte[] rowCounts = countRowTypes(puts);
    ...
    public void testPartialDeleteIndicesRejectsInvalidTableId() throws Exception {
        factory.deleteIndices(-1, row, new HashMap<String, IndexSchema>());
    }
    @Test(expected = TableNotFoundException.class)
    public void testPartialDeleteIndicesRejectsUnknownTableId() throws Exception {
        factory.deleteIndices(99, row, ImmutableMap.of(INDEX1, INDICES.get(INDEX1)));
    }
    @Test(expected = NullPointerException.class)
    public void testPartialDeleteIndicesRejectsNullRow() throws Exception {
        factory.deleteIndices(1, null, new HashMap<String, IndexSchema>());
    }
