...
public class SequentialLoadIncrementalHFiles extends Configured {
    private static Log LOG = LogFactory.getLog(SequentialLoadIncrementalHFiles.class);
    ...
    public SequentialLoadIncrementalHFiles(Configuration conf) throws Exception {
        super(conf);
        this.cfg = conf;
    ...
    public void doBulkLoad(Path hfofDir, final HTable table)
        ...
        try {
            ...
            while (!queue.isEmpty()) {
                ...
                bulkLoadPhase(table, conn, queue, regionGroups);
            }
     ...
     * groupOrSplitPhase.
     */
    protected void bulkLoadPhase(final HTable table, final HConnection conn, Deque<LoadQueueItem> queue,
                                 final Multimap<ByteBuffer, LoadQueueItem> regionGroups) throws IOException {
        // get all the results.
        for (Map.Entry<ByteBuffer, ? extends Collection<LoadQueueItem>> e : regionGroups.asMap().entrySet()) {
            try {
                ...
                LOG.error("Exception thrown during bulk load.", ex);
            }
        }
    }
        ...
        while (!queue.isEmpty()) {
            ...
            final Callable<List<LoadQueueItem>> call = new Callable<List<LoadQueueItem>>() {
                public List<LoadQueueItem> call() throws Exception {
                    return groupOrSplit(regionGroups, item, table, startEndKeys);
                }
            };
    ...
    protected List<LoadQueueItem> groupOrSplit(Multimap<ByteBuffer, LoadQueueItem> regionGroups,
        ...
        int idx = Arrays.binarySearch(startKeys, first,
        ...
        boolean lastKeyInRange =
                Bytes.compareTo(last, endKeys[idx]) < 0 || Bytes.equals(endKeys[idx], HConstants.EMPTY_BYTE_ARRAY);
        if (!lastKeyInRange) {
            List<LoadQueueItem> lqis = splitStoreFile(item, table,
                    startKeys[indexForCallable],
                    endKeys[indexForCallable]);
            return lqis;
        }

        // group regions.
        regionGroups.put(ByteBuffer.wrap(startKeys[idx]), item);
        return null;
    }
