...
void zero_time(MYSQL_TIME* time)
{
...
uchar* create_key_copy(Field* index_field, const uchar* key, uint* key_len,
		THD* thd)
{
	...
	switch (index_field_type)
	{
		...
		case MYSQL_TYPE_YEAR:
		{
			...
			bytes_to_long((uchar *) &int_val, sizeof(uint32_t), false,
					key_copy);
			*key_len = sizeof(long long);
			make_big_endian(key_copy, *key_len);
			break;
		...
		case MYSQL_TYPE_NEWDATE:
		{
			...
			switch (index_field_type)
			{
				...
				case MYSQL_TYPE_NEWDATE:
					if (*key_len == 3)
					{
						extract_mysql_newdate((long) uint3korr(key),
								&mysql_time);
					}
					else
					{
						extract_mysql_old_date((int32) uint4korr(key),
								&mysql_time);
					}
					break;
				case MYSQL_TYPE_TIMESTAMP:
					...
					break;
				case MYSQL_TYPE_DATETIME:
					extract_mysql_datetime((ulonglong) uint8korr(key),
							&mysql_time);
					break;
			}
